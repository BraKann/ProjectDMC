#!/usr/bin/env python3
"""Benchmark fanout avec asyncio et aiohttp - suppression et peuplement automatiques via subprocess"""
import asyncio
import aiohttp
import csv
import time
...
POSTS_PER_USER = 100  # nombre fixe de posts par utilisateur
...
# ======================================
def clear_and_seed(users, posts_per_user, followers):
    ...
    print(f"🌱 Peuplement avec {posts_per_user} posts/user et {followers} followees/user...")
    try:
        result = subprocess.run([
            "python3", SEED_SCRIPT,
            "--users", str(users),
            "--posts", str(posts_per_user),
...
# ======================================
def main():
    print("=== Benchmark Fanout (asyncio + aiohttp) ===")
    ...
    with open(OUTPUT_FILE, "w", newline="") as f:
        ...
        for fanout in FANOUT_LEVELS:
            print(f"\n=== Test avec {fanout} followees/user ===")

            if not clear_and_seed(1000, POSTS_PER_USER, followers=fanout):
            ...
            for run in range(1, 4):
                ...
                writer.writerow([fanout, f"{avg:.2f}", run, failed])
                f.flush()
                time.sleep(1)
